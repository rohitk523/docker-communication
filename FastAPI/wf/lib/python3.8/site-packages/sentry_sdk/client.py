...
def _get_options(*args, **kwargs):
    ...
    if args and (
        isinstance(args[0], (text_type, bytes, str)) or args[0] is None
    ):
...
class _Client(object):
            ...
            if event_ is None:
                if self.transport:
                    self.transport.record_lost_event(
                        "event_processor",
                        data_category=(
                            "transaction" if is_transaction else "error"
                        ),
                    )
                return None
    ...
    def _is_ignored_error(self, event, hint):
        ...
        for ignored_error in self.options["ignore_errors"]:
            ...
            if isinstance(ignored_error, string_types):
                if (
                    ignored_error == error_full_name
                    or ignored_error == error_type_name
                ):
                    return True
            else:
    ...
    def _should_capture(
        ...
        ignoring_prevents_recursion = (
            scope is not None and not scope._should_capture
        )
    ...
    def _should_sample_error(
        ...
        if not_in_sample_rate:
            ...
            if self.transport:
                self.transport.record_lost_event(
                    "sample_rate", data_category="error"
                )
        ...
        if is_transaction or attachments:
            ...
            if is_transaction:
                if "profile" in event_opt:
                    event_opt["profile"]["transaction_id"] = event_opt[
                        "event_id"
                    ]
