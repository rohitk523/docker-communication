...
class RavenResolver(object):
    ...
    def _simplify(self, pattern):
        ...
        result = self._optional_group_matcher.sub(
            lambda m: "%s" % m.group(1), pattern
        )

        # handle named groups first
        result = self._named_group_matcher.sub(
            lambda m: "{%s}" % m.group(1), result
        )
    ...
    def _resolve(self, resolver, path, parents=None):
        ...
        for pattern in resolver.url_patterns:
            ...
            prefix = "".join(
                self._simplify(get_regex(p).pattern) for p in parents
            )
