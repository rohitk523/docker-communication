...
class HttpTransport(Transport):
    ...
    def __init__(
        ...
        self._worker = BackgroundWorker(
            queue_size=options["transport_queue_size"]
        )
    ...
    def _fetch_pending_client_report(self, force=False, interval=60):
        ...
        if not (
            force or self._last_client_report_sent < time.time() - interval
        ):
            return None
    ...
    def _flush_client_reports(self, force=False):
        # type: (bool) -> None
        client_report = self._fetch_pending_client_report(
            force=force, interval=60
        )
    ...
    def _in_no_proxy(self, parsed_dsn):
        ...
        for host in no_proxy.split(","):
            host = host.strip()
            if parsed_dsn.host.endswith(host) or parsed_dsn.netloc.endswith(
                host
            ):
                return True
        return False
...
def make_transport(options):
    ...
    elif isinstance(ref_transport, type) and issubclass(
