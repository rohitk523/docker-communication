...
def extract_tracestate_data(header):
    ...
    if header:
        ...
        if sentry_match:
            ...
            before, after = map(
                lambda s: s.strip(","), header.split(sentry_entry)
            )
            ...
            if not re.search(
                "^{b64}$".format(b64=base64_stripped), sentry_value
            ):
                sentry_entry = None
        else:
            after = header
        ...
        third_party_entry = (
            ",".join(filter(TRACESTATE_ENTRIES_REGEX.search, [before, after]))
            or None
        )
