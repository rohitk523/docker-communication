        ...
        if decoded == user.username:
            ...
            if file_details.filename in all_files:
                ...
                if db.query(models.keywords).filter(models.keywords.user_id == user.uuid).count() == 1:
                    db.query(models.keywords).filter(models.keywords.user_id == user.uuid).delete()
                    db.add(file_details)
                    db.commit()
...
def upload(file: UploadFile = File(), db : Session = Depends(get_db),credentials: HTTPAuthorizationCredentials = Security(security)):
    if file.filename[-3:] != 'txt':
        ...
        if decoded == user.username:
            ...
            x = requests.post('http://textfile:8086/requests', json={'name':user.username,'filename': file_details.filename,'time': str(datetime.now().strftime("%c")),'wordcount': total})
            print(x.text)
            return wordcount
        else:
            return HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid token')
